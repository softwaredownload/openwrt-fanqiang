...
```
一般情况下，你只要修改 overtls 监听的端口成实际值就行。如果你本机 overtls 监听的端口和上面配置文件中的相同，你可以直接使用这个配置
...
- `sudo`
   ...
   申请管理员权限。在 windows 10 和以上平台上，可以自行安裝 sudo 程序來模拟 Linux 的类似行为，首先安裝 gsudo
